...
use KoninklijkeCollective\MyUserManagement\Service\BackendUserControllerService;
...
#[Controller]
final class BackendUserController extends ActionController
{
    ...
    public function indexAction(Demand $demand = null, int $currentPage = 1, string $operation = ''): ResponseInterface
    {
        ...
        $demand = $this->getBackendUserControllerService()->manipulateDemandForUser($demand, $this->getFlashMessageQueue());
        ...
        if ($this->getBackendUserControllerService()->canCreate('index')) {
            $addUserButton = $buttonBar->makeLinkButton()
                ...
                ->setTitle(LocalizationUtility::translate('LLL:EXT:cms-beuser/Resources/Private/Language/locallang.xlf:btn.backendUser.create'))
        ...
        $shortcutButton = $buttonBar->makeShortcutButton()
            ->setRouteIdentifier('myusermanagement_user_admin')
    ...
    {
        $this->getBackendUserControllerService()->validateAccessToAction('filemounts');
    ...
    protected function getBackendUserControllerService(): BackendUserControllerService
    {
        static $backendUserController;
        if ($backendUserController) {
            return $backendUserController;
        }

        return $backendUserController = GeneralUtility::makeInstance(BackendUserControllerService::class)
